const _config = {
  ...
  weatherLat: String(process.env.WEATHER_LAT ?? "59.3293"),
  weatherLon: String(process.env.WEATHER_LON ?? "18.0686"),
  weatherUnits: String(process.env.WEATHER_UNITS ?? "metric"),
  weatherLang: String(process.env.WEATHER_LANG ?? "en"),
  cacheDuration: Number(process.env.CACHE_DURATION ?? 3600),
  apiTimeout: Number(process.env.API_TIMEOUT ?? 10_000),
